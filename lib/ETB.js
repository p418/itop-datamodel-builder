...
ETB.pipe = function()
{
	if(this.stream == void 0)
		throw new ReferenceError('Stream not found. Did you called .from() first?');
...
ETB.bundle = function()
{
	...
	var mainStream = this.stream, stream = Through2.obj();

	mainStream.on('data', function(files)
	{
		//console.log(files.length+" files received");
		files.forEach(function(file){ stream.push(file); });
		stream.end();
	});
